iopctrl = function() {
    ...
    iopctrl.LedButton = function() {
        ...
        function LedButton(g) {
            g.each(function() {
                var g = d3.select(this);

                var LedButton = g.selectAll("rect").data([ 0 ]), 
                    ledUpdate = (
                        LedButton.enter().append("rect")
                            .attr("class", on ? "on" : "off")
                            .attr("width", width)
                            .attr("height", height)
                            .attr("rx", round)
                            .style("fill", on ? colorOn : colorOff), d3.transition(LedButton)
                            .on("click", function() { 
                                g.selectAll("rect").attr("class", on ? "on" : "off")
                            }
                        )
                    );

                ledExit = d3.transition(LedButton.exit()).style("opacity", 1e-6).remove();
                _ledUpdate = ledUpdate;
            });
        }
        LedButton.on = function(x) {
        ...
        LedButton.width = function(x) {
            if (!arguments.length) return width;
            width = x;
            return LedButton;
        };
        LedButton.height = function(x) {
            if (!arguments.length) return height;
            height = x;
            return LedButton;
        };
        LedButton.round = function(x) {
            if (!arguments.length) return round;
            round = x;
            return LedButton;
        };
        LedButton.color = function(x) {
            if (!arguments.length) return colorOn;
            colorOn = d3.rgb(x);
            colorOff = colorOn.darker(2);
            return LedButton;
        };
        LedButton.transitionDuration = function(x) {
